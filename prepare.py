...
def preprocess(document_text):
    #remove the leading numbers and make everything lowercase
...
#save the vocab in txt file
with open('tf-idf/vocab.txt','w') as f:
    for key in vocab.keys():
        f.write("%s\n" % key)

#save the idf values in a txt file
with open('tf-idf/idf-values.txt','w') as f:
    for key in vocab.keys():
        f.write("%s\n" % vocab[key]) 
        
#save the documents in txt file
...
#save the inverted index in txt file
with open('tf-idf/inverted_index.txt','w') as f:
